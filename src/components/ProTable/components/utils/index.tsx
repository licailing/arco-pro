...
import { getValueByPath } from '@arco-design/web-vue/es/_utils/get-value-by-path';
...
const columRender = ({
  ...
  const text = String(getValueByPath(record, item.dataIndex) ?? '');
...
export function mergePagination<T>(
  ...
  const { current, pageSize, total } = pageInfo.value;
